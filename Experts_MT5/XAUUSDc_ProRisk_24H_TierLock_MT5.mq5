//+------------------------------------------------------------------+
//|                                  XAUUSDc_ProRisk_24H_TierLock_MT5.mq5   |
//|  Risk-managed XAUUSD scalping EA for MetaTrader 5                 |
//|  No profit guarantee. Demo/backtest before live trading.          |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "XAUUSDc MT5 24H EA with EMA/RSI/Stochastic scalping, ATR SL/TP, tiered profit lock, equity guard, Telegram."
...
input long            InpMagicNumber             = 24052027;
...
input bool            InpUseTieredProfitLock     = true;            // Lock SL step-by-step like tiered TP
input double          InpTier1R                  = 1.00;            // At +1R lock SL to entry + Tier1LockR
input double          InpTier1LockR              = 0.20;
input double          InpTier2R                  = 1.50;
input double          InpTier2LockR              = 0.75;
input double          InpTier3R                  = 2.00;
input double          InpTier3LockR              = 1.25;
input double          InpTier4R                  = 2.50;
input double          InpTier4LockR              = 1.75;
input double          InpTier5R                  = 3.00;
input double          InpTier5LockR              = 2.25;
input double          InpTier6R                  = 4.00;
input double          InpTier6LockR              = 3.00;
...
void OpenTrade(const int type)
{
   ...
   bool ok = false;
   if(type == ORDER_TYPE_BUY) ok = trade.Buy(lots, g_symbol, 0.0, sl, tp, "XAUUSDc MT5 ProRisk 24H TierLock");
   if(type == ORDER_TYPE_SELL) ok = trade.Sell(lots, g_symbol, 0.0, sl, tp, "XAUUSDc MT5 ProRisk 24H TierLock");
...
void MonitorPositionsHard()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(InpUseTieredProfitLock)
      {
         double tierSl = GetTieredProfitStop(posType, openPrice, initialRisk, profitDistance);
         if(tierSl > 0.0 && IsBetterStop(posType, tierSl, newSl))
         {
            newSl = tierSl;
            shouldModify = true;
         }
...
double GetTieredProfitStop(const ENUM_POSITION_TYPE posType,
                           const double openPrice,
                           const double initialRisk,
                           const double profitDistance)
{
   double lockR = 0.0;

   if(profitDistance >= initialRisk * InpTier1R) lockR = InpTier1LockR;
   if(profitDistance >= initialRisk * InpTier2R) lockR = InpTier2LockR;
   if(profitDistance >= initialRisk * InpTier3R) lockR = InpTier3LockR;
   if(profitDistance >= initialRisk * InpTier4R) lockR = InpTier4LockR;
   if(profitDistance >= initialRisk * InpTier5R) lockR = InpTier5LockR;
   if(profitDistance >= initialRisk * InpTier6R) lockR = InpTier6LockR;

   if(lockR <= 0.0) return(0.0);

   double lockDistance = initialRisk * lockR;
   if(posType == POSITION_TYPE_BUY) return(openPrice + lockDistance);
   return(openPrice - lockDistance);
}
...
void LogStatus(const string message)
{
   ...
   Print("[XAUUSDc 24H TierLock] ", message);
}
...
void UpdateChartPanel()
{
   ...
   Comment("XAUUSDc ProRisk 24H TierLock MT5\n",
